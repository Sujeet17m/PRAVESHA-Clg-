...
import themeImage from '../assets/hero.png'; // Replace with your cyber_dragon_theme.png
...
const Hero: React.FC = () => {
  ...
  return (
    <section id="home" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', paddingTop: 'clamp(5rem, 12vw, 7rem)', paddingBottom: '3rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 2, width: '100%' }}>
        <div className="hero-grid">
          ...
          <div style={{ maxWidth: '800px' }}>
            ...
            <h1 ref={h1Ref} style={{ opacity: 0, fontSize: 'clamp(2.5rem, 7vw, 5rem)', lineHeight: 1.1, marginBottom: '0.9rem', color: '#fff' }}>
              NATIONAL LEVEL<br />
              SYMPOSIUM<br />
            ...
            <p ref={paraRef} style={{ opacity: 0, fontSize: '1.2rem', color: 'var(--text-secondary)', marginBottom: '1.5rem', borderLeft: '4px solid var(--neon-red)', paddingLeft: '1.2rem', lineHeight: 1.7, maxWidth: '600px' }}>
              Synthesizing Technology, Celebrating Talent.<br />The ultimate engineering technical symposium.
            </p>
            {/* Info chips */}
            <div ref={chipsRef} style={{ opacity: 0, display: 'flex', flexDirection: 'row', gap: '0.75rem', marginBottom: '1.5rem', maxWidth: '500px' }}>
              {[
                ...
                <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '0.6rem 1rem', flex: 1, minWidth: 0 }}>
                  {item.icon}
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', letterSpacing: '1.5px' }}>{item.label}</div>
                    <div style={{ fontWeight: 700, fontSize: 'clamp(0.8rem, 2.5vw, 1rem)', fontFamily: 'Orbitron', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.value}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Countdown */}
            <div ref={countdownRef} style={{ opacity: 0, marginBottom: '2rem', maxWidth: '500px' }}>
              <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', letterSpacing: '3px', marginBottom: '0.75rem' }}>EVENT STARTS IN</div>
              <div className="countdown-row">
                {[{ val: days, label: 'DAYS' }, { val: hours, label: 'HRS' }, { val: minutes, label: 'MIN' }, { val: seconds, label: 'SEC' }].map(({ val, label }) => (
                  <div key={label} className="countdown-box" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>
      ...
      <div ref={chevronRef}
        style={{ position: 'absolute', bottom: '1.5rem', left: '50%', transform: 'translateX(-50%)', color: 'var(--text-secondary)', cursor: 'pointer', zIndex: 3 }}
